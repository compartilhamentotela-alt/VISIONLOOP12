      ...
      try {
        ...
        const videos = playlist.videos.map(item => {
          if (typeof item === 'string') {
            return { name: item, duration: 0, isImage: IMAGE_EXT_REGEX.test(item) };
          }
          return item;
        });
        playlists[id] = {
    ...
    writeStream.on("finish", () => {
      ...
      transcodeVideoToHD(destPath, tmpOutputPath)
        .then(() => {
          ...
          // do arquivo original — que ainda está no disco neste momento. Usar
          // getUniqueFilename() aqui faria o arquivo virar "Nome (1).mp4" em
          // TODO upload .mp4, mesmo sem nenhum conflito real. Nesse caso
          ...
          const mp4Name = desiredMp4Name === finalName
            ? finalName
