...
function broadcastToControllers(data) {
  ...
  controllers.forEach((ws) => { if (ws.readyState === 1) ws.send(msg); });
}

function broadcastTvList() {
  const list = Array.from(tvs.entries()).map(([code, t]) => ({
    code,
    name: t.name,
    video: t.video,
  ...
  broadcastToControllers({ type: "tv_list", tvs: list });
}
...
wss.on("connection", (ws) => {
  ws.on("message", (raw) => {
    ...
    if (msg.type === "tv_connect") {
      ...
      tvs.set(code, { ws, name: msg.name || `TV ${tvs.size + 1}`, video: null, playlist: null, paused: false });
      ws._tvCode = code;
