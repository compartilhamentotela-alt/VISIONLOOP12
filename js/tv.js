...
function connect() {
  ...
  ws.onopen = () => {
    setConn(true);
    ws.send(JSON.stringify({ type: 'tv_connect', name: tvName() }));
  };
  ...
  ws.onmessage = (e) => {
    ...
    if (msg.type === 'play') {
      playlistMode = false; currentPlaylist = null;
