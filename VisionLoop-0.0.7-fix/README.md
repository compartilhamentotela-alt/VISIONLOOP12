...
### 2. Adicione seus vídeos e imagens

Envie os arquivos pelo controlador (aba "Vídeos") ou coloque-os direto na pasta `videos/`:
- Vídeos aceitos no upload: `.mp4`, `.webm`, `.mkv`, `.mov`, `.avi`
- Imagens aceitas no upload: `.jpg`, `.jpeg`, `.png`, `.gif`, `.webp`
...
- **Envio em fila (um arquivo por vez)**: se você selecionar ou arrastar vários arquivos de uma só vez, eles **não** sobem todos juntos. O controlador monta uma fila e envia um de cada vez — o próximo só começa quando o anterior termina de subir e de ser convertido no servidor. Isso evita saturar a rede local e sobrecarregar o servidor com várias conversões simultâneas. A barra de progresso de cada arquivo aparece normalmente na lista; os que ainda estão na fila aguardam a vez.
...
**Enviei vários arquivos e eles estão subindo devagar / um de cada vez**
→ Isso é o comportamento esperado. O envio é sequencial de propósito: subir e converter vários vídeos ao mesmo tempo satura a rede local e o servidor, deixando *todos* os uploads mais lentos e sujeitos a falha. Na fila, cada arquivo termina de verdade antes do próximo começar. O tempo total costuma ser igual ou menor do que enviando tudo em paralelo.
