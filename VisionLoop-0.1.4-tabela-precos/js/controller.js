...
// Envia o Txitens.txt pro servidor, que acha o preço de cada um dos 20
// cortes fixos e desenha as 2 imagens da tabela (a partir do modelo em
// assets/tabela-template.png). As imagens já saem salvas junto com os
// outros vídeos/imagens, prontas pra selecionar e enviar a qualquer TV.
function gerarTabelaPrecos(file) {
  const input = document.getElementById('fileInput-tabelaPrecos');
  const btn = document.getElementById('btnGerarTabelaPrecos');
  const statusEl = document.getElementById('tabelaPrecosStatus');
  if (!file) return;

  if (!/\.txt$/i.test(file.name)) {
    statusEl.innerHTML = '<span style="color:#dc2626;">Escolha um arquivo .txt (o Txitens.txt exportado do PDV/balança).</span>';
    if (input) input.value = '';
    return;
  }

  btn.disabled = true;
  btn.textContent = '⏳ Gerando tabela...';
  statusEl.textContent = 'Lendo o arquivo e desenhando as imagens — isso pode levar alguns segundos...';

  fetch('/gerar-tabela-precos', {
    method: 'POST',
    ...
    .then(({ ok, data }) => {
      if (!ok) throw new Error(data.error || 'Erro ao gerar a tabela.');

      const totalPreenchidos = data.encontrados.length + (data.duplicados ? data.duplicados.length : 0);
      const partes = [];
      partes.push(`✅ Tabela gerada: <strong>${data.arquivos.length}</strong> imagem(ns) — ${totalPreenchidos} de 20 preços preenchidos.`);

      if (data.duplicados && data.duplicados.length) {
        const lista = data.duplicados.map(d => `${d.nome} (achei ${d.precosEncontrados.map(p => 'R$ ' + p.toFixed(2).replace('.', ',')).join(' e ')} cadastrados no PDV — usei o maior, R$ ${d.precoUsado.toFixed(2).replace('.', ',')}; confira se está certo)`).join('; ');
        partes.push(`<span style="color:#d97706;">⚠️ Preço duplicado no Txitens.txt (usado o maior automaticamente): ${lista}.</span>`);
      }
      if (data.naoEncontrados && data.naoEncontrados.length) {
        partes.push(`<span style="color:#d97706;">⚠️ Não encontrado no Txitens.txt (deixado em branco): ${data.naoEncontrados.join(', ')}.</span>`);
      }
    ...
    .finally(() => {
      btn.disabled = false;
      btn.textContent = '📄 Gerar tabela a partir do Txitens.txt';
      if (input) input.value = '';
    });
}
