...
Nova função pedida diretamente no controlador: gerar a tabela de preços de carnes bovinas (que antes só existia como script Python rodado fora do app) direto pela interface web, com um botão.

### O que mudou

- **Botão "🥩 Tabela de Preços — Carnes Bovinas"** na aba Vídeos (tela inicial, sem TV selecionada). Ao escolher o arquivo `Txitens.txt` (exportado do PDV/balança), o servidor gera as 2 imagens da tabela de preços já com os valores preenchidos, e elas aparecem direto na lista normal de vídeos/imagens — prontas pra mandar pra qualquer TV, sem precisar rodar nenhum script fora do app.
- **Os nomes dos cortes nunca são desenhados pelo servidor** — partem prontos das 2 imagens-base `assets/tabela-bovinos-1.png` e `assets/tabela-bovinos-2.png` (mesmas imagens já usadas antes). A cada geração, o servidor só escreve em cima delas: o título **"Bovinos"** no espaço em branco de cima, e o preço de cada corte no espaço em branco reservado de cada linha.
- **Busca de preço por igualdade exata**, não por "contém": cada um dos 20 cortes fixos tem uma descrição exata esperada (ex.: "Carne Bovina Picanha"). Isso evita pegar o preço errado quando existem descrições parecidas com valores diferentes no arquivo (ex.: "Costela Gaúcha" x "Costela Gaúcha Temperada").
- **Cadastro duplicado no PDV → usa o maior preço**: quando a mesma descrição aparece em mais de uma linha do Txitens.txt com preços diferentes (caso real encontrado em teste: "Carne Bovina Patinho" com 2 códigos de produto e preços distintos no mesmo arquivo), o preço é preenchido normalmente usando sempre o **maior** valor entre os encontrados (regra definida pelo usuário) — e o controlador avisa quais cortes foram resolvidos assim, com os preços encontrados, pra permitir conferência manual no PDV.
- **Descrição não encontrada → fica em branco**: se a descrição exata esperada não aparece em nenhuma linha do arquivo, o espaço daquele corte fica em branco (nunca inventa um preço) e o controlador lista quais cortes precisam de conferência manual.
- Gerar de novo **substitui** as 2 imagens anteriores (mesmo nome de arquivo sempre) — não fica acumulando versões antigas na lista de mídia.
- Nova rota `POST /gerar-tabela-precos` no servidor; novo arquivo `lib/tabela-precos.js` com toda a lógica de leitura do txt e desenho do título/preços (usando `@napi-rs/canvas`, escolhido por não exigir compilação nativa no servidor — diferente do `node-canvas` tradicional, que costuma dar problema em ambientes como o Render).
- Novos arquivos de apoio: `assets/tabela-bovinos-1.png` e `assets/tabela-bovinos-2.png` (as 2 imagens-base, com os nomes já prontos) e `assets/fonts/DejaVuSans-Bold.ttf` (fonte usada no texto desenhado, empacotada junto pra não depender de fontes instaladas no servidor).
- Nova dependência: `@napi-rs/canvas`. **Depois de atualizar os arquivos, é preciso rodar `npm install` de novo** antes de reiniciar/reimplantar o servidor.
