...
**Versão 0.1.4** — 05/08/2026 · O histórico de mudanças fica em [CHANGELOG.md](CHANGELOG.md).
...
| 🥩 Tabela de Preços (Carnes Bovinas) | Gera automaticamente as 2 imagens da tabela de preços a partir do Txitens.txt exportado do PDV/balança — ver seção abaixo |

---

## 🥩 Tabela de Preços (Carnes Bovinas)

Na aba **Vídeos**, na tela inicial (sem nenhuma TV selecionada), tem um bloco **"🥩 Tabela de Preços — Carnes Bovinas"** com o botão **"📄 Gerar tabela a partir do Txitens.txt"**.

Como funciona:

1. Clique no botão e escolha o arquivo `Txitens.txt` exportado do PDV/balança do açougue.
2. O servidor parte das 2 imagens-base já prontas (`assets/tabela-bovinos-1.png` e `assets/tabela-bovinos-2.png`, com os nomes dos 20 cortes já desenhados permanentemente) — **os nomes nunca são redesenhados**. Em cima delas, o servidor só escreve o título **"Bovinos"** no espaço em branco de cima e o preço de cada corte (Picanha, Alcatra, Contra Filé, Coxão Mole, Coxão Duro, Patinho, Fraldinha, Maminha, Filé Mignon, Cupim, Acém, Lagarto, Músculo, Costela Gaúcha, Peixinho, Rabada, Fígado, Peito, Coração e Chambari) no espaço em branco reservado de cada linha.
3. As 2 imagens já aparecem na lista normal de vídeos/imagens (mesmo nome sempre — gerar de novo **substitui** a versão anterior) e podem ser enviadas a qualquer TV como qualquer outro conteúdo.

O que aparece no aviso depois de gerar:

- **Preços preenchidos automaticamente** — quando o texto exato do corte foi achado sem duplicidade no Txitens.txt.
- **⚠️ Preço duplicado (usado o maior)** — quando existe mais de uma linha no Txitens.txt com a mesma descrição mas preços diferentes (cadastro duplicado no PDV — aconteceu de verdade com "Carne Bovina Patinho" no arquivo de teste, com 2 códigos de produto diferentes). Nesses casos o preço **é preenchido normalmente, usando sempre o maior valor encontrado** — mas o aviso mostra os preços encontrados, pra você conferir no seu PDV se o maior é mesmo o correto.
- **⚠️ Não encontrado** — quando nenhuma linha do Txitens.txt bate com a descrição esperada daquele corte. Nesse caso o espaço de preço fica em branco.

Isso é proposital: a função nunca inventa um preço quando não tem certeza — ela sempre avisa e deixa o espaço em branco pra revisão manual.

**Requisito de instalação:** essa função depende do pacote `@napi-rs/canvas` (adicionado ao `package.json` nesta versão). Depois de atualizar os arquivos, rode `npm install` de novo antes de subir/reiniciar o servidor — sem isso o botão aparece, mas a geração falha avisando que a dependência não foi instalada.

Se um dia precisar trocar o layout ou os nomes dos cortes, é só substituir os arquivos `assets/tabela-bovinos-1.png` e `assets/tabela-bovinos-2.png` por novas imagens-base (mantendo o espaço em branco de cima e o espaço em branco à direita de cada linha) — não é preciso mexer em código.
