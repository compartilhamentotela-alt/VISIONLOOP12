...
// Gerador da tabela de preços de carnes (lê o Txitens.txt e desenha os
// preços nas imagens do modelo). Depende do pacote "@napi-rs/canvas" — se
// por algum motivo ele não tiver sido instalado (ex.: `npm install` antigo
// antes dessa função existir), o require falha aqui, mas isso não derruba
// o servidor: só desativa essa função específica (ver rota mais abaixo).
let tabelaPrecos = null;
try {
  tabelaPrecos = require("./lib/tabela-precos");
} catch (err) {
  console.error("Função de tabela de preços indisponível (dependência @napi-rs/canvas não instalada?):", err.message);
}
...
const server = http.createServer((req, res) => {
  ...
  // Gera a tabela de preços de carnes bovinas: recebe o conteúdo do
  // Txitens.txt no corpo da requisição, acha o preço de cada um dos 20
  // cortes fixos e desenha 2 imagens prontas (a partir do modelo em
  // assets/tabela-template.png), salvando-as junto com os outros vídeos/
  // imagens — assim elas aparecem na lista normal e podem ser mandadas pra
  // qualquer TV como qualquer outra imagem.
  if (req.method === "POST" && urlPath === "/gerar-tabela-precos") {
    if (!tabelaPrecos) {
      res.writeHead(501, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: "Essa função depende do pacote @napi-rs/canvas, que não está instalado no servidor. Rode \"npm install\" de novo (ele foi adicionado ao package.json) e reinicie/reimplante." }));
      return;
    }
    const chunks = [];
    ...
    req.on("end", () => {
      (async () => {
        try {
          const txtContent = Buffer.concat(chunks).toString("latin1");
          if (!txtContent.trim()) {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ error: "Arquivo vazio ou não recebido." }));
            return;
          }
          ...
          const resultado = await tabelaPrecos.gerarTabelasPrecos(txtContent, dir);

          // Com R2 ligado, sobe as 2 imagens pro bucket (sobrescrevendo a
          // versão anterior, se já existir) e limpa a cópia local — igual ao
          // fluxo normal de upload. Sem R2, elas já ficaram salvas certas em
          // /videos.
          if (R2_ENABLED) {
            for (const img of resultado.imagens) {
              await uploadToR2(img.localPath, img.key);
          ...
          res.end(JSON.stringify({
            success: true,
            arquivos: resultado.imagens.map((i) => i.key),
          ...
          res.writeHead(500, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: "Erro ao gerar a tabela de preços. Veja os logs do servidor para detalhes." }));
        }
      })();
    });
    return;
